#!/usr/bin/env python3
"""
ASD Complete Dashboard FIXED
- Parents Dashboard: Graphs, ABA, DTT, TEACCH + Chatbot
- Child Dashboard: Better AI Chat (ABA-based) + Games
- All games working
"""
...
ABA_RESPONSES = {
    "greeting": ["Hello! 😊 How are you today? I'm so happy to see you!", "Hi there! 🌟 It's great to talk with you!", "Hey! 💫 I'm Pepper, your friend! How can I help you today?"],
    "positive": ["Great job! 🌟 I'm so proud of you!", "Excellent! 👏 You're doing amazing!", "Wow! 🎉 That's wonderful!", "Keep going! 💪 You're doing great!"],
    "feeling_happy": ["I'm so happy you're feeling happy! 😊 Your smile makes my day brighter!", "Yay! 🎉 Happiness is wonderful! Tell me what made you happy!"],
    "feeling_sad": ["It's okay to feel sad. 🤗 I'm here with you. Let's take a deep breath together.", "I understand. 💙 Would you like to talk about it? I'm here to listen."],
    "feeling_angry": ["It's okay to feel angry. 🧘 Let's count to 5 together. 1...2...3...4...5. You're doing great!", "I understand. Let's take a break and breathe together."],
    "help": ["I'm here to help you! 💪 What do you need? You can do this!", "Of course! 🌟 Let me help you. Tell me more about what you need."],
    "game": ["Great idea! 🎮 Let's play a game! Look at the 'My Games' section below!", "I love games! 🎲 Choose a game from the list and have fun! Tell me about it after!"],
    "task": ["You're doing great with your tasks! ✅ Keep going!", "I'm proud of you for completing your tasks! 🌟"],
    "default": ["That's interesting! 🎉 Tell me more!", "I love learning from you! 💙 What else would you like to talk about?", "You're doing wonderful! 🌟 Keep talking to me!"]
}
...
def get_aba_response(message):
    ...
    elif any(w in msg_lower for w in ["sad", "upset", "unhappy", "down"]):
...
# ==================== Parent Chatbot Knowledge ====================
PARENT_KNOWLEDGE = {
    "what is autism": "Autism Spectrum Disorder (ASD) is a developmental condition that affects communication, behavior, and social interaction. Each person with autism is unique.",
    "signs of autism": "Common early signs include: delayed speech, avoiding eye contact, not responding to name, repetitive movements, sensory sensitivities.",
    ...
    "dtt therapy": "DTT (Discrete Trial Training) breaks skills into small steps. Each trial has instruction, response, and consequence.",
    ...
    "sensory overload": "Signs include covering ears, hiding, crying. Help by reducing stimuli, moving to a quiet space, offering deep pressure.",
    "positive reinforcement": "Praise specific behaviors. Use rewards immediately. Be consistent. Focus on what the child did well.",
    "routine importance": "Routines provide predictability and reduce anxiety. Use visual schedules. Warn before transitions."
}

def get_parent_response(question):
    q = question.lower()
    for key, answer in PARENT_KNOWLEDGE.items():
        if key in q:
            return answer
    return "I'm here to help! Ask me about autism signs, ABA therapy, DTT, TEACCH, sensory overload, meltdowns, or routines."
...
<body>
    <div class="card">
        ...
        <a href="/parent/register" class="link">Register</a>
...
<body>
    <div class="card">
        ...
        <a href="/parent/login" class="link">← Back</a>
    </div>
</body>
</html>
'''
...
<body>
    <div class="card">
        ...
        <a href="/child/register" class="link">Register</a>
        <a href="/" class="link">← Back</a>
        {% if error %}<div class="error">{{ error }}</div>{% endif %}
    </div>
</body>
...
<body>
    <div class="card">
        ...
        <a href="/child/login" class="link">← Back</a>
    </div>
</body>
</html>
'''
...
<body>
    <div class="card">
        ...
        <a href="/parent/dashboard" class="link">← Back</a>
    </div>
</body>
</html>
'''
    ...
    <div class="dashboard">
        ...
        <div class="card" id="chatCard">
            ...
            <div class="chat-area" id="chatArea">
                <div class="message bot-message">Hello! I'm your Autism Support Assistant. Ask me about autism, ABA, DTT, TEACCH, sensory overload, or meltdowns.</div>
            </div>
            <div class="chat-input">
                <input type="text" id="chatInput" placeholder="Ask about autism..." onkeypress="if(event.key==='Enter') sendParentMessage()">
                <button onclick="sendParentMessage()">Send</button>
            </div>
            <div class="quick-actions">
                <div class="quick-btn" onclick="parentQuick('what is autism')">What is Autism?</div>
                <div class="quick-btn" onclick="parentQuick('aba therapy')">ABA Therapy</div>
                <div class="quick-btn" onclick="parentQuick('how to handle meltdown')">Meltdown Help</div>
                <div class="quick-btn" onclick="parentQuick('sensory overload')">Sensory Overload</div>
            </div>
        </div>
    ...
    <script>
        ...
        function selectChild(childName) {
            ...
            fetch(`/parent/child_progress/${childName}`)
                .then(res => res.json())
                .then(data => {
                    document.querySelectorAll('#progressCard, #attentionChartCard, #abaChartCard, #gamesCard').forEach(c => c.style.display = 'block');
...
<body>
    ...
    <div class="dashboard">
        <div class="card">
            ...
            <div class="chat-area" id="chatArea">
                <div class="message bot-message">Hi {{ username }}! I'm Pepper! 😊 I'm here to help you learn and have fun! How are you feeling today? 🌟</div>
            </div>
            <div class="chat-input">
...
def parent_chat():
    ...
    response = get_parent_response(question)
...
def play_game(game_name):
    # Try to find and render the game template
    from flask import render_template
    try:
        return render_template(f'games/{game_name}.html')
    except:
        # If template not found, show games list
        games_html = '<div class="games-grid">'
        for g in GAMES_LIST:
            games_html += f'<div class="game-card" onclick="window.location.href=\'/game/{g}\'"><div style="font-size:32px;">{ICONS.get(g, "🎮")}</div><div>{g.replace("_", " ")}</div></div>'
        games_html += '</div>'
        return f'''
        <!DOCTYPE html>
        <html>
        <head><title>Games</title><style>
            ...
            .game-card {{ background: rgba(255,255,255,0.1); border-radius: 15px; padding: 20px; text-align: center; cursor: pointer; }}
            .game-card:hover {{ background: #e94560; }}
        </style></head>
        <body><h1>🎮 Educational Games</h1><a href="/child/dashboard" style="color:#e94560;">← Back</a>{games_html}</body>
        </html>
        '''

if __name__ == '__main__':
    print("\n" + "="*50)
    print("🧩 ASD COMPLETE DASHBOARD FIXED")
    print("="*50)
    print("✅ Parents Dashboard - Graphs + Chatbot")
    print("✅ Child Dashboard - ABA-based AI Chat")
